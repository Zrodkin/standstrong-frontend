...
const ConfirmationModal = ({ isOpen, onClose, onConfirm, classTitle, isLoading }) => {
    // (Modal code remains the same as your provided version)
                ...
                >
                    <div className="p-6 sm:p-8">
                        ...
                        <p className="text-slate-600 mb-6">
                            You are about to register for the class: <strong className="text-slate-800">{classTitle}</strong>.
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-end space-y-3 sm:space-y-0 sm:space-x-3">
                            <button
                                onClick={onClose}
                                disabled={isLoading}
                                className="px-4 py-2 border border-slate-300 rounded-md text-slate-700 hover:bg-slate-100 transition-colors w-full sm:w-auto disabled:opacity-50"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={onConfirm}
                                disabled={isLoading}
                                className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors w-full sm:w-auto flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {isLoading ? (
                                    <>
...
const ClassDetailPage = () => {
    ...
    const fetchClassData = useCallback(async () => {
        // Reset states **before** starting the fetch
        setError(null);
        ...
        try {
            ...
            setClassData(data);
            // setLoading(false); // Set loading false only on success *if* not using finally
        } catch (err) {
            console.error("Fetch Class Error:", err);
            setError(err.response?.data?.message || 'Failed to load class details. Please try again.');
            // setLoading(false); // Set loading false on error *if* not using finally
        } finally {
            // This block should *always* run if the promise settles (resolves or rejects)
            setLoading(false);
            nprogress.done();
        }
    }, [id, fetchClassById]); // Dependencies are correct

    useEffect(() => {
        fetchClassData();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [id]); // Re-fetch only if ID changes. fetchClassData is memoized.
    ...
    const handleImageError = (e) => {
        // Simplified: Just hide the image if it fails to load main or fallback.
    ...
    // --- Registration Logic ---
    // (handleOpenConfirmation and handleConfirmRegister remain the same)
    ...
    const handleConfirmRegister = async () => {
        ...
        setRegistering(true);
        setRegisterError(null);
        nprogress.start(); // Start progress for registration action

        try {
            await registerClass(id);
            setRegisterSuccess(true);
            setIsConfirming(false);
            // Refresh data slightly delayed
            setTimeout(fetchClassData, 300);
        } catch (err) {
            console.error("Registration Error:", err);
            setRegisterError(err.response?.data?.message || 'Registration failed. Please try again.');
            setIsConfirming(false); // Close modal even on error
        } finally {
            setRegistering(false);
            nprogress.done(); // Stop progress for registration action
        }
    };

    // --- Helpers ---
    // (isUserRegistered, formatTime, googleMapsUrl remain the same)
    ...
    // Check if classData is available before creating the URL
    ...
    // 2. Handle Error State (loading is false now)
    if (error) {
        return <ErrorMessage message={error} onRetry={fetchClassData} />;
    }

    // 3. Handle No Data Found (loading is false, no error, but no data)
    if (!classData) {
        // This case means the fetch likely completed successfully but returned no data (e.g., 404)
        return <ErrorMessage message="Class not found or could not be loaded." onRetry={() => navigate('/classes')} />;
    }

    // 4. Data is available, proceed with rendering the page
    const alreadyRegistered = isUserRegistered();
    const isExternal = classData.registrationType === 'external';
    const schedule = classData.schedule || [];
    const availableSlots = classData.maxCapacity ? classData.maxCapacity - (classData.registeredStudents?.length || 0) : Infinity; // Handle undefined capacity
    const isFull = classData.maxCapacity != null && availableSlots <= 0;


    return (
        <div className="bg-slate-50 min-h-screen">
            {/* Breadcrumb */}
            {/* (Breadcrumb JSX remains the same) */}
            ...
            {/* Hero Section */}
             {/* (Hero Section JSX remains the same, uses handleImageError) */}
                    ...
                    <div className="lg:sticky lg:top-6 space-y-6">
                        <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 space-y-5">
                            ...
                            <hr className="border-slate-100"/>
                             ...
                             {/* Availability */}
                            {classData.maxCapacity != null && ( // Check if maxCapacity is defined
                                <div className="flex items-start space-x-4">
                                    <FiUsers className="text-blue-500 mt-1 flex-shrink-0 h-5 w-5" />
                                    <div>
                                        <p className="text-sm font-semibold text-slate-500 uppercase tracking-wide">Availability</p>
                                        <p className={`text-slate-800 font-medium ${isFull && !alreadyRegistered ? 'text-red-600' : availableSlots === Infinity ? 'text-green-600' : ''}`}>
                                            {isFull && !alreadyRegistered ? 'Class Full' :
                                             availableSlots === Infinity ? 'Slots Available' : // Handle infinite slots
                             ...
                             <div className="pt-5">
                                ...
                                {alreadyRegistered ? (
                                    ...
                                    <button
                                        type="button"
                                        disabled={true}
                                        className="w-full inline-flex justify-center items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-slate-500 bg-slate-200 cursor-not-allowed"
                                    >
                                        Class Full
                                    </button>
                                 ) : (
            ...
            <ConfirmationModal
                ...
                classTitle={classData.title} // Ensure classData exists here
            ...
            <footer className="bg-slate-800 text-slate-400 text-center py-8 mt-16 sm:mt-24 border-t border-slate-700">
                 <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-sm">
                    © {new Date().getFullYear()} Your Company Name. All rights reserved. {/* Replace with your actual company name */}
                </div>
            </footer>
        </div>
    );
